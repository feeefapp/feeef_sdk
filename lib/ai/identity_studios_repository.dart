...
/// Identity Studio projects (`/identityStudios`).
///
/// Each studio groups many [image_generations] rows (board versions + cut-outs).
class IdentityStudiosRepository {
  IdentityStudiosRepository({required this.client});
  ...
  Future<Map<String, dynamic>> find(String id) async {
    final response = await client.get('/identityStudios/$id');
    final body = Map<String, dynamic>.from(response.data as Map);
    final data = body['data'];
    if (data is Map) return Map<String, dynamic>.from(data);
    return body;
  ...
  Future<void> delete(String id) async {
    await client.delete('/identityStudios/$id');
  }
  ...
  generations({
    ...
    final response = await client.get(
      '/identityStudios/$studioId/generations',
  ...
  Future<Map<String, dynamic>> setPrimary({
    ...
    final response = await client.patch(
      '/identityStudios/$studioId/primaries',
